...
public class CalendarView extends View  {
	...
	private  Calendar caculateCalendar=Calendar.getInstance();
	private String[] dutyPerson=new String[42];
	public  String[] duty={"甲","乙","丙","丁","戊"};
...
//		dutyCalendar.set(dutyCalendar.HOUR,0);
//		dutyCalendar.set(dutyCalendar.MINUTE,1);
//		dutyCalendar.set(dutyCalendar.SECOND,0);
		//init();
		invalidate();
	}
...
Log.i("info", "landscape"); // 横屏
	...
	surface.cellTextSize = (surface.height - surface.height*13/400) / 13f * 0.5f;
...
Log.i("info", "portrait"); // 竖屏
				surface.width = (int) (getResources().getDisplayMetrics().widthPixels);
				surface.height = (int) (getResources().getDisplayMetrics().heightPixels*4/5);
				surface.cellTextSize = (surface.height - surface.height*13/400) / 13f * 0.35f;
}
...
//		canvas.drawRect(surface.weekHeight, surface.width, surface.weekHeight, surface.width, surface.cellBgPaint);
		...
		for (int i = 0; i < 42; i++) {
            ...
            if (dutyPerson[i].equals(duty[dutyOrder])){
                color=surface.todayNumberColor;
            }
			drawBanCellText(canvas,i,dutyPerson[i],color);
		}
	...
	private int dutyIndex(float time,int order){
		...
		Log.d("days", days+"");
//		Log.d("daysnow",""+ new SimpleDateFormat("yyyy-MM-dd HH:mm:ss:SS").format(calendar.getTime()));
//		Log.d("daysduty",""+ new SimpleDateFormat("yyyy-MM-dd HH:mm:ss:SS").format(dutyCalendar.getTime()));
		Log.d("dutyL", length+"");

		if (days>=0)
			return  (days+order)%length;
		else
			return (length+(days-1+order)%length)%length;
	...
	private void calculateDate() {
		...
		if (monthStart > 0) {
			...
			Log.d("DAY_OF_MONTH1",""+ new SimpleDateFormat("yyyy-MM-dd HH:mm:ss:SS").format(dutyCalendar.getTime()));
						...
						for (int i=1;i<42;i++){
							dutyDay++;
							dutyPerson[i]=duty[dutyDay%length];
	...
	private void drawBanCellText(Canvas canvas, int index, String text, int color) {
		int x = getXByIndex(index);
		int y = getYByIndex(index)+1;
		surface.datePaint.setColor(color);
		float cellY = surface.monthHeight + surface.weekHeight + (y - 1)
				* surface.cellHeight + surface.cellHeight * 3 / 4f;
		float cellX = (surface.cellWidth * (x - 1))
...
//		}
	...
	private class Surface {
		...
		public void init() {
			...
			weekHeight = (float) ((temp + temp * 0.3f) * 0.7)/2;
			cellHeight = (height - monthHeight - weekHeight) / 12f;
			cellWidth = width / 7f;
...
borderWidth=5;
			...
			/////////////////////////////////////////////////////////
			 ...
			 // 应用mask
			 borderPaint.setMaskFilter(emboss);
			///////////////////////////////////////////////////////////////
			...
			for(int i=0;i<7;i++){
				...
				boxPath.rLineTo(0, monthHeight + weekHeight + 12 * cellHeight);
			}
			for(int i=0;i<13;i++){
